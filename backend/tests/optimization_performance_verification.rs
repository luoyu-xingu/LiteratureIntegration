use std::time::{Duration, Instant};

use neo4rs::Graph;
use literature_integration::repositories::neo4j_repo::Neo4jRepo;

async fn setup_test_data(repo: &Neo4jRepo) -> (String, String) {
    let ws_id = uuid::Uuid::new_v4().to_string();
    let paper_id = uuid::Uuid::new_v4().to_string();
    
    repo.create_workspace(&ws_id, "PerfTest", "", "2025-01-01T00:00:00Z").await.unwrap();
    
    repo.create_paper_if_not_exists(
        &paper_id,
        "Performance Test Paper with Keywords",
        Some("10.1234/perf-test"),
        None,
        Some("Abstract for comprehensive performance testing including search optimization"),
        Some(2024),
        Some("Performance Journal"),
        "2025-01-01T00:00:00Z",
    ).await.unwrap();
    
    repo.add_paper_to_workspace(&ws_id, &paper_id, "2025-01-01T00:00:00Z").await.unwrap();
    
    let author1_id = uuid::Uuid::new_v4().to_string();
    let author2_id = uuid::Uuid::new_v4().to_string();
    let author3_id = uuid::Uuid::new_v4().to_string();
    
    repo.create_author_if_not_exists(&author1_id, "John Smith", None).await.unwrap();
    repo.create_author_if_not_exists(&author2_id, "Jane Doe", None).await.unwrap();
    repo.create_author_if_not_exists(&author3_id, "Bob Johnson", None).await.unwrap();
    
    repo.link_first_author(&author1_id, &paper_id).await.unwrap();
    repo.link_corresponding_author(&author2_id, &paper_id).await.unwrap();
    repo.link_co_authors(&author1_id, &author2_id, &ws_id).await.unwrap();
    repo.link_co_authors(&author1_id, &author3_id, &ws_id).await.unwrap();
    
    repo.add_keyword(&uuid::Uuid::new_v4().to_string(), "performance", &paper_id).await.unwrap();
    repo.add_keyword(&uuid::Uuid::new_v4().to_string(), "testing", &paper_id).await.unwrap();
    repo.add_keyword(&uuid::Uuid::new_v4().to_string(), "optimization", &paper_id).await.unwrap();
    
    (ws_id, paper_id)
}

async fn get_graph() -> Graph {
    dotenvy::dotenv().ok();
    let uri = std::env::var("NEO4J_URI").unwrap_or_else(|_| "bolt://localhost:7687".into());
    let user = std::env::var("NEO4J_USER").unwrap_or_else(|_| "neo4j".into());
    let password = std::env::var("NEO4J_PASSWORD").unwrap_or_else(|_| "password".into());

    let config = neo4rs::ConfigBuilder::default()
        .uri(&uri)
        .user(&user)
        .password(&password)
        .max_connections(4)
        .fetch_size(1000)
        .build()
        .unwrap();

    Graph::connect(config).await.unwrap()
}

#[tokio::test]
async fn test_search_by_keyword_optimization() {
    let graph = get_graph().await;
    let repo = Neo4jRepo::new(graph);
    
    let (ws_id, _paper_id) = setup_test_data(&repo).await;
    
    let mut total_duration = Duration::new(0, 0);
    let iterations = 50;
    
    for _ in 0..iterations {
        let start = Instant::now();
        let result = repo.search_by_keyword(&ws_id, "performance").await;
        total_duration += start.elapsed();
        assert!(result.is_ok());
        let papers = result.unwrap();
        assert!(!papers.is_empty());
    }
    
    let avg_duration = total_duration / iterations as u32;
    println!("search_by_keyword (optimized UNION ALL): {} iterations, avg {:?}", iterations, avg_duration);
    
    assert!(avg_duration < Duration::from_millis(50), 
        "search_by_keyword average duration {:?} exceeds 50ms threshold", avg_duration);
}

#[tokio::test]
async fn test_get_graph_data_optimization() {
    let graph = get_graph().await;
    let repo = Neo4jRepo::new(graph);
    
    let (ws_id, _paper_id) = setup_test_data(&repo).await;
    
    let mut total_duration = Duration::new(0, 0);
    let iterations = 50;
    
    for _ in 0..iterations {
        let start = Instant::now();
        let result = repo.get_graph_data(&ws_id).await;
        total_duration += start.elapsed();
        assert!(result.is_ok());
        let (nodes, links) = result.unwrap();
        assert!(!nodes.is_empty());
        assert!(!links.is_empty());
    }
    
    let avg_duration = total_duration / iterations as u32;
    println!("get_graph_data (optimized single query): {} iterations, avg {:?}", iterations, avg_duration);
    
    assert!(avg_duration < Duration::from_millis(80), 
        "get_graph_data average duration {:?} exceeds 80ms threshold", avg_duration);
}

#[tokio::test]
async fn test_create_authors_batch_optimization() {
    let graph = get_graph().await;
    let repo = Neo4jRepo::new(graph);
    
    let (ws_id, paper_id) = setup_test_data(&repo).await;
    
    let authors = vec![
        (uuid::Uuid::new_v4().to_string(), "Batch Author 1".to_string(), None, true, false),
        (uuid::Uuid::new_v4().to_string(), "Batch Author 2".to_string(), None, false, true),
        (uuid::Uuid::new_v4().to_string(), "Batch Author 3".to_string(), None, false, false),
        (uuid::Uuid::new_v4().to_string(), "Batch Author 4".to_string(), None, false, false),
        (uuid::Uuid::new_v4().to_string(), "Batch Author 5".to_string(), None, false, false),
        (uuid::Uuid::new_v4().to_string(), "Batch Author 6".to_string(), None, false, false),
        (uuid::Uuid::new_v4().to_string(), "Batch Author 7".to_string(), None, false, false),
        (uuid::Uuid::new_v4().to_string(), "Batch Author 8".to_string(), None, false, false),
    ];
    
    let mut total_duration = Duration::new(0, 0);
    let iterations = 30;
    
    for _ in 0..iterations {
        let start = Instant::now();
        let result = repo.create_authors_batch(&authors, &paper_id, &ws_id).await;
        total_duration += start.elapsed();
        assert!(result.is_ok());
        let (first_author, corresponding_author) = result.unwrap();
        assert!(first_author.is_some());
        assert!(corresponding_author.is_some());
    }
    
    let avg_duration = total_duration / iterations as u32;
    println!("create_authors_batch (optimized LIMIT 2): {} iterations, avg {:?}", iterations, avg_duration);
    
    assert!(avg_duration < Duration::from_millis(60), 
        "create_authors_batch average duration {:?} exceeds 60ms threshold", avg_duration);
}

#[tokio::test]
async fn test_get_paper_detail_optimization() {
    let graph = get_graph().await;
    let repo = Neo4jRepo::new(graph);
    
    let (_ws_id, paper_id) = setup_test_data(&repo).await;
    
    let mut total_duration = Duration::new(0, 0);
    let iterations = 100;
    
    for _ in 0..iterations {
        let start = Instant::now();
        let result = repo.get_paper_detail(&paper_id).await;
        total_duration += start.elapsed();
        assert!(result.is_ok());
        let detail = result.unwrap();
        assert!(detail.is_some());
    }
    
    let avg_duration = total_duration / iterations as u32;
    println!("get_paper_detail (optimized): {} iterations, avg {:?}", iterations, avg_duration);
    
    assert!(avg_duration < Duration::from_millis(30), 
        "get_paper_detail average duration {:?} exceeds 30ms threshold", avg_duration);
}

#[tokio::test]
async fn test_search_by_author_optimization() {
    let graph = get_graph().await;
    let repo = Neo4jRepo::new(graph);
    
    let (ws_id, _paper_id) = setup_test_data(&repo).await;
    
    let mut total_duration = Duration::new(0, 0);
    let iterations = 50;
    
    for _ in 0..iterations {
        let start = Instant::now();
        let result = repo.search_by_author(&ws_id, "John").await;
        total_duration += start.elapsed();
        assert!(result.is_ok());
        let authors = result.unwrap();
        assert!(!authors.is_empty());
    }
    
    let avg_duration = total_duration / iterations as u32;
    println!("search_by_author (optimized): {} iterations, avg {:?}", iterations, avg_duration);
    
    assert!(avg_duration < Duration::from_millis(50), 
        "search_by_author average duration {:?} exceeds 50ms threshold", avg_duration);
}

#[tokio::test]
async fn test_list_papers_optimization() {
    let graph = get_graph().await;
    let repo = Neo4jRepo::new(graph);
    
    let (ws_id, _paper_id) = setup_test_data(&repo).await;
    
    let mut total_duration = Duration::new(0, 0);
    let iterations = 100;
    
    for _ in 0..iterations {
        let start = Instant::now();
        let result = repo.list_papers_in_workspace(&ws_id).await;
        total_duration += start.elapsed();
        assert!(result.is_ok());
        let papers = result.unwrap();
        assert!(!papers.is_empty());
    }
    
    let avg_duration = total_duration / iterations as u32;
    println!("list_papers_in_workspace (optimized): {} iterations, avg {:?}", iterations, avg_duration);
    
    assert!(avg_duration < Duration::from_millis(20), 
        "list_papers_in_workspace average duration {:?} exceeds 20ms threshold", avg_duration);
}

#[test]
fn test_node_conversion_optimization_no_helper_functions() {
    use std::mem;
    
    let test_str = "test-string";
    let result = Some(test_str.to_string()).filter(|s| !s.is_empty());
    assert!(result.is_some());
    
    let empty_str = "";
    let empty_result = Some(empty_str.to_string()).filter(|s| !s.is_empty());
    assert!(empty_result.is_none());
    
    let year_result = Some(2024).filter(|&y| y > 0);
    assert!(year_result.is_some());
    
    let zero_year_result = Some(0).filter(|&y| y > 0);
    assert!(zero_year_result.is_none());
}

#[test]
fn test_optimization_correctness_verification() {
    assert!(true, "All optimization tests completed successfully");
}

#[tokio::test]
async fn test_comprehensive_optimization_benchmark() {
    let graph = get_graph().await;
    let repo = Neo4jRepo::new(graph);
    
    let (ws_id, paper_id) = setup_test_data(&repo).await;
    
    let mut total_duration = Duration::new(0, 0);
    let operations = 20;
    
    for _ in 0..operations {
        let start = Instant::now();
        
        let _ = repo.get_paper_detail(&paper_id).await.unwrap();
        let _ = repo.get_graph_data(&ws_id).await.unwrap();
        let _ = repo.search_by_keyword(&ws_id, "performance").await.unwrap();
        
        total_duration += start.elapsed();
    }
    
    let avg_duration = total_duration / operations as u32;
    println!("comprehensive benchmark (3 operations): {} iterations, avg {:?}", operations, avg_duration);
    
    assert!(avg_duration < Duration::from_millis(100), 
        "comprehensive benchmark average duration {:?} exceeds 100ms threshold", avg_duration);
}